...
const Contador = require("../modules/contador") // Importar el modelo de contador


// Función para validar el tiempo mínimo para citas (6 horas)
const validarTiempoMinimo = (fechaHoraCita) => {
  const ahora = new Date()
  const tiempoDiferencia = fechaHoraCita.getTime() - ahora.getTime()
  const horasDiferencia = tiempoDiferencia / (1000 * 60 * 60)

  return horasDiferencia >= 6
}

// Función para combinar fecha y hora en un objeto Date
const combinarFechaHora = (fechaStr, horaStr) => {
  // Si la fecha ya es un objeto Date, extraer la parte de fecha
  const fechaBase =
    typeof fechaStr === "string" ? fechaStr.split("T")[0] : new Date(fechaStr).toISOString().split("T")[0]

  // Crear un nuevo objeto Date combinando fecha y hora
  return new Date(`${fechaBase}T${horaStr}`)
}

// Modificar el controlador crearCita
const crearCita = async (req, res) => {
  try {
    const { nombreempleado, nombrecliente, fechacita, horacita, duracionTotal, montototal, estadocita, servicios } =
      req.body
    ...
    // Crear fecha y hora combinadas como objeto Date
    const fechaHoraCita = combinarFechaHora(fechacita, horacita)

    // Validar tiempo mínimo para todos los servicios (6 horas)
    if (!validarTiempoMinimo(fechaHoraCita)) {
      return res.status(400).json({
        message: "Las citas deben agendarse con al menos 6 horas de anticipación",
      })
    }
    ...
    const nuevaCita = new Cita({
      ...
      horacita: fechaHoraCita, // Guardar como Date
      duracionTotal,
      montototal,
...
const obtenerCitas = async (req, res) => {
  try {
    const citas = await Cita.find()
      ...
      .populate("servicios.servicio", "nombreServicio precio") // Asegúrate de que los campos existan en el modelo Servicio
    res.json({ citas })
  } catch (error) {
...
const obtenerCitaPorId = async (req, res) => {
  try {
    ...
    const cita = await Cita.findById(id)
      ...
      .populate("servicios.servicio", "nombreServicio precio") // Asegúrate de hacer populate para los servicios
    if (!cita) {
...
const actualizarCita = async (req, res) => {
  try {
    ...
    // Si se proporciona fecha y hora, combinarlas
    let fechaHoraActualizada
    if (fechacita && horacita) {
      fechaHoraActualizada = combinarFechaHora(fechacita, horacita)

      // Validar tiempo mínimo solo si la cita no está en progreso o completada
      const citaExistente = await Cita.findById(id)
      if (
        citaExistente &&
        citaExistente.estadocita !== "En Progreso" &&
        citaExistente.estadocita !== "Completada" &&
        !validarTiempoMinimo(fechaHoraActualizada)
      ) {
        return res.status(400).json({
          message: "Las citas deben agendarse con al menos 6 horas de anticipación",
        })
    ...
    const cita = await Cita.findByIdAndUpdate(
      id,
      {
        nombreempleado,
        nombrecliente,
        fechacita: fechaHoraActualizada || undefined,
        horacita: fechaHoraActualizada || undefined,
        montototal,
        estadocita,
...
const cancelarCita = async (req, res) => {
  try {
    ...
    const cita = await Cita.findByIdAndUpdate(
      ...
      .populate("nombreempleado", "nombreempleado")
      .populate("nombrecliente", "nombrecliente")
      .populate("servicios.servicio", "nombreServicio precio")
...
const verificarDisponibilidad = async (req, res) => {
  try {
    ...
    const citasEmpleado = await Cita.find({
      ...
      horacita: { $gte: fechaInicio, $lte: fechaFin }, // Usar horacita como Date
    ...
    const ahora = new Date()
    const tiempoMinimo = 6 * 60 * 60 * 1000 // 6 horas en milisegundos
    ...
    for (let hora = 9; hora < 19; hora++) {
      for (const minuto of [0, 30]) {
        ...
        // Verificar si cumple con el tiempo mínimo de 6 horas
        const tiempoDiferencia = horaInicio.getTime() - ahora.getTime()
        if (tiempoDiferencia < tiempoMinimo) {
          continue // Saltar este horario si no cumple con el tiempo mínimo
        }
        ...
        for (const cita of citasEmpleado) {
          const citaInicio = new Date(cita.horacita)
          ...
          if (cita.servicios && cita.servicios.length > 0) {
            ...
            const citaServiciosInfo = await Servicio.find({ _id: { $in: cita.servicios.map((s) => s.servicio) } })
          ...
          citaFin.setMinutes(citaInicio.getMinutes() + citaDuracion)
...
const obtenerCitasPorCliente = async (req, res) => {
  try {
    ...
    let citas = await Cita.find({ nombrecliente: clienteId })
      ...
      .sort({ horacita: 1 }) // Ordenar por horacita
